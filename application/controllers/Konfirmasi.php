...
class Konfirmasi extends CI_Controller {
	...
	public function unotify(){
		...
		$datax = array('pembayaran'=>$status_trx,'status_trx'=>cetak($this->input->post('status')),'penampung'=>cetak($this->input->post('via')));
		$where = array('catatan' =>cetak($this->input->post('trx_id')));
		$this->model_app->update('rb_penjualan_otomatis', $datax, $where);

		$idp = $this->db->query("SELECT kode_transaksi FROM rb_penjualan_otomatis where catatan='".cetak($this->input->post('trx_id'))."'")->row_array();
